...
* Использование свойства DOM-объекта (тоже не является хорошей практикой);
...
Еще один способ установки обработчиков событий представляет использование *слушателей событий* (от англ. event listeners), который имеет преимущества по сравнению с более старыми механизмами, рассмотренными ранее. Этот способ позволяет повесить множество обработчиков на один элемент.
...
* **passive** – указывает, что обработчик никогда не вызовет *preventDefault()* – это также рассмотрим позже;
...
Этот метод принимает аргументы тех же типов, что *addEventListener*. При этом для того, чтобы убрать обработчик нужно указывать **точно такие же аргументы**, которые мы использовали при его назначении. Рассмотрим на примере:
...
> Обратите внимание! **Удаление требует именно ту же функцию**, которая была назначена. 
...
// Назначили обработчик
element.addEventListener('click', sayHello.bind(null, 'Вася'));

// Обработчик не будет удален
element.removeEventListener('click', sayHello.bind(null, 'Вася'));
```

В примере выше функция, которая передана в *addEventListener*, не совпадает с функцией в removeEventListenter, т.к. вызов *sayHello.bind(null, 'Вася')* каждый раз возвращает новую функцию, поэтому удаления обработчика не происходит. Чтобы его удалить, нужно чтобы **ссылка** на функцию была той же самой:
...
// Создали ссылку на функцию
const clickHandler = sayHello.bind(null, 'Вася');
...
Для некоторых элементов браузер выполняет действие по умолчанию при срабатывании определенных событий. Например, клик по ссылке инициирует переход на новый URL, но зачастую это бывает не нужно. В JavaScript возможно отменить стандартное действие браузера, для этого используется метод *preventDefault*.
...
```js
link.addEventListener('click', function (event) {
    // Отмены действия по умолчанию не произойдет
    event.preventDefault();
}, {passive: true});
```
...
Можно сказать, что это похоже на то, как всплывает пузырек воздуха в воде. Отловить событие на стадии всплытия можно при вызове *addEventListener* без указания третьего аргумента:

```js
elem.addEventListener(event, handler);

// тоже самое, что
elem.addEventListener(event, handler, {capture: false});

// или просто "false", как сокращение для {capture: false}
...
Перехват (погружение) довольно редко используется в разработке, но в некоторых сценариях может быть полезно. Чтобы поймать событие на стадии погружения, нужно у метода *addEventListener* в третьем аргументе установить *capture* в значение *true*:
...
Для чего это бывает нужно? Допустим, есть задача определить был ли клик по веб-странице, но на каком то из дочерних элементов этой страницы было предотвращено всплытие события *click*:

```html
<body>
    <div onclick="event.stopPropagation();">Дочерний элемент</div>
</body>

<script>
    document.addEventListener('click', () => {
       alert('По веб-странице кликнули!')
    });
</script>
```

В данном случае при клике по *div* не будет вызван *alert*, соответственно, определить был ли клик по веб-странице мы не сможем. Для решения этой задачи можно поймать событие на стадии погружения, переписав код следующим образом:
