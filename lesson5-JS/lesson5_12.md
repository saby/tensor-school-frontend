...
Создать объект предельно просто, причем JavaScript предлагает сразу несколько вариантов, как это можно сделать. Для большинства ситуацийы подходит простейший способ, но в ряде случаев его оказывается недостаточно и тогда можно использовать другие варианты.
...
Объект по своей сути - это хэш-таблица. Т.е. структура данных, реализующая интерфейс ассоциативного массива.
Проще говоря, объект в JavaScript - это словарь, который позволяет хранить пары ключ + значение и выполнять три операции: операцию добавления новой пары, операцию поиска и операцию удаления пары по ключу.
