...
Существует возможность задать параметры по умолчанию. В этом случае, если параметр не передан, он получит некое дефолтное значение:
...
Независимо от того, сколько параметров указано при объявлении функции, при вызове функции можно передавать в нее *любое* количество параметров. Если передано меньше параметров, чем указано при объявлении функции, недостающие параметры, как было описано выше, получат значение undefined. Если же передано больше параметров, чем указано при объявлении функции, это не вызовет ошибку, а доступ к ним можно получить при помощи массива *arguments*.
...
function getMax() {
    let max = 0;
    // Получаем доступ к переданным параметрам через предопределенный массив arguments
...
Тем не менее, подобный подход нельзя отнести к хорошему стилю программирования. Более корректный подход состоит в том, чтобы явно описать все ожидаемые параметры, а если их число и порядок заранее неизвестны, то правильнее было бы передавать параметры в виде массива произвольной длины или объекта с произвольным числом свойств. Это сделает код понятнее.
...
```javascript
let имя = function [еще_имя]([параметры]) {
    // Тело функции
    [return результат;]
};
```
...
Например, стрелочная нотация лексически связывает значение this. Кроме того, стрелочные функции всегда анонимны.
