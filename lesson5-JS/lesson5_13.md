...
В простейшем случае, когда функция вызывается самым "стандартным" способом, *this* будет ссылаться на глобальный объект. Для кода, который выполняется на странице браузера, это будет объект *window*:
...
Рассмотрим чуть более сложный случай, когда функция вызывается как метод объекта.
...
Методы *.call()* и *.apply()* используются для вызова функции с настраиваемым контекстом:
