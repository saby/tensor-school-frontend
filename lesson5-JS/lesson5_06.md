...
При этом явного деления на целочисленне и вещественные числа в JavaScript нет.
...
Аналогичным образом, функция **parseFloat(string)** позволяет преобразовывать переданную строку в вещественное значение.
...
Неявное преобразование числа к строке произойдет при сложении числа и сроки:
...
Обратите внимание, что когда в примере выше мы применяли toPrecision() или toFixed() по отношению к целочисленому значению 60, нам пришлось заставить интерпретатор выполнить неявное приведение типов с помощью круглых скобок. В противном случае попытка вызвать эти функции привела бы к ошибке, т.к. целочисленные значения не имеют подобных методов округления.

Для получения длинны строки используется свойство length.
...
Можно сделать это явным образом, используя объект new Boolean(), либо (как обычно и делают разработчики) - с помощью '!!'.
...
console.log(new Boolean(count2));   // false
```
...
Если переменная объявлена, но не инициализирована – ее значение undefined.
